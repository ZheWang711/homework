...
* Prototype: minimum set of data required to correctly predict all non-outlier points.
* Absorbed points:non-outlier points, and could be correctly predicted using prototype points.

What a CNN algorithm typically do is to find out the prototype points

(b): The prototype selection algorithm pseudo-code is listed as follow:
```
def prototype_selection(training_data, M)
    original = training_data
    prototype = [random_choice(original)]  # a list that will be used to store prototype points
    outlier = []    # a list that will be used to store outlier points
    ...
    # Second, keep expanding prototype until get a fixed point (prototype no longer expand)
    # After the while loop, prototype will become the set of prototypes and original will become the set of absorbed points
    flag = True # if there is new data added in this iteration
    while flag:
        if len(prototype) == M:
            break
        flag = False
        for pt in range(original):
            prototype_training = training_data[prototype]   # training_data rows filter by prototypes
            label = the predicted label of pt based on prototype_training using 1NN method
